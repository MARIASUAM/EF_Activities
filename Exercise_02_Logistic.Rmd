...
for(t in 2:NT){
  df$n_1.95[t] = df$n_1.95[t-1] + r_1.95*df$n_1.95[t-1]*(1-df$n_1.95[t-1]/K)
  df$n_2.05[t] = df$n_2.05[t-1] + r_2.05*df$n_2.05[t-1]*(1-df$n_2.05[t-1]/K)
  df$n_2.5[t] = df$n_2.5[t-1] + r_2.5*df$n_2.5[t-1]*(1-df$n_2.5[t-1]/K)
  df$n_2.8[t] = df$n_2.8[t-1] + r_2.8*df$n_2.8[t-1]*(1-df$n_2.8[t-1]/K)
  df$n_minus_1[t] = df$n_minus_1[t-1] + r_minus_1*df$n_minus_1[t-1]*(1-df$n_minus_1[t-1]/K)
}

# library(ggplot2)

ggplot(data = df, aes(x = df$NT)) +
  geom_line(aes(y = n_1.95, colour = 'r = 1.95')) +
  geom_line(aes(y = n_2.05, colour = 'r = 2.05')) +
  geom_line(aes(y = n_2.5, colour = 'r = 2.5')) +
  geom_line(aes(y = n_2.8, colour = 'r = 2.8')) +
  geom_line(aes(y = n_1, colour = 'r = 1')) +
  geom_line(aes(y = n_minus_1, colour = 'r = -1'))
```
...
n = c(10,100,1000,10000)	# sequence of sample sizes
for(i in 1:4){			# loop over these sample sizes
  hist(rlogis(n[i]),main=n[i],probability=TRUE,breaks=40)  
				#here breaks defines number of bins in the histogram
  lines(x,dlogis(x),col=2)
}
```
...
n = c(10,100,1000,10000)	# sequence of sample sizes
for(i in 1:4){			# loop over these sample sizes
  hist(rbinom(n[i], n[i], 0.3),main=n[i],probability=TRUE,breaks=40)  #here breaks defines number of bins in the histogram
  lines(n[i],dbinom(n[i], n[i], 0.3),col=2) # why is not working???
}
```
...
# María
# log-normal     | lnorm  | meanlog, sdlog
x <- rlnorm(n=10000, meanlog=0,sdlog=0.5)
y <- sin(x)
...
```{r}
# María
indexes <- sample(NE,10)
n_df <- as.data.frame(t(n))

ggplot(data = n_df, aes(x = 1:NT)) +
  geom_line(aes(y = n_df[,indexes[1]])) +
  geom_line(aes(y = n_df[,indexes[2]])) +
  geom_line(aes(y = n_df[,indexes[3]])) +
  geom_line(aes(y = n_df[,indexes[4]])) +
  geom_line(aes(y = n_df[,indexes[5]])) +
  geom_line(aes(y = n_df[,indexes[6]])) +
  geom_line(aes(y = n_df[,indexes[7]])) +
  geom_line(aes(y = n_df[,indexes[8]])) +
  geom_line(aes(y = n_df[,indexes[9]])) +
  geom_line(aes(y = n_df[,indexes[10]]))
```
...
# María
pop_forecast <- n[,15]
hist(pop_forecast, main = "Population forecast, time 15")
```
...
ggplot(df, aes(x = time)) +
  ...
  geom_line(aes(y = CI_low, colour = 'CI')) +
  geom_line(aes(y = CI_high, colour = 'CI'))

plot(df$time, df$median, type = 'l')
ciEnvelope(df$time, df$CI_low, df$CI_high)
```
...
n.stats = apply(n,2,quantile,c(0.025,0.5,0.975))
plot(1:NT, n.stats[2,], type = 'l', main = paste("r = ", r, sep = ""))
ciEnvelope(1:NT, n.stats[1,], n.stats[3,])
```
