...
# Run the model
nr_chains <- 3
j.model.trees <- jags.model (file = textConnection(NormalMeanN),
                             data = data, n.chains = nr_chains)

# Sample from the model
jags.out.trees <- coda.samples (model = j.model.trees,
                          variable.names = c("mu"), n.iter = 5000)

# Visual assessment of MCMC outputs
plot(jags.out.trees) # MCMC history on the left, parameter density on the right side

# Table of summary statistics for the posterior estimate of the mean (no burning and thinning applied)
summary(jags.out.trees)

# Burn-in analysis
GBR <- gelman.plot(jags.out)
burnin = 100
jags.burn.trees <- window(jags.out.trees,start=burnin)
plot(jags.burn.trees) # MCMC history on the left, parameter density on the right side after burn-in has been discarded

# Correlation
acfplot(jags.burn.trees)

# Effective sample size
effectiveSize(jags.burn.trees)

# Length of your MCMC (i.e. the total number of “updates”)
??
  
# Settings: the number of chains, burnin
nr_chains
burnin
```
...
```{r}
# María
# Model definition: S as gamma distributed? s1 and s2 values??
NormalMeanN_unknownVariance <- "
model {
  mu ~ dnorm(mu0,T) # prior on the mean 
...
# Run the model
nr_chains <- 3
j.model.trees.unkownvar <- jags.model (file = textConnection(NormalMeanN_unknownVariance), data = data, n.chains = nr_chains)
data$s1 <- 0.1
data$s2 <- 0.1

# Sample from the model
jags.out.trees.unkownvar <- coda.samples (model = j.model.trees.unkownvar, variable.names = c("mu", "S"), n.iter = 5000)

plot(jags.out.trees.unkownvar)
summary(jags.out.trees.unkownvar)
```
