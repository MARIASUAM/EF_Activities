...
# Parameter covariances and parameter correlation matrix
var.burn.mat      <- as.matrix(var.burn)
pairs(var.burn.mat)	## to evaluate parameter correlation
cor(var.burn.mat)  

# Compare the summary statistics for the Bayesian regression model to those from the classical regression:  summary(lm( y ~ x1 )).  This should include a comparison of the means and uncertainties of **all 3 model parameters**

classical_reg <- lm( y ~ x1 )
summary(classical_reg) # can we see the prec (or sd) here?
...
y <- rnorm(n,x%*%beta,sigma)
plot(x1,y)
plot(x2,y) # María: why to use same y for different x?
```
...
model{
  ...
  beta ~ dmnorm(c(b0, b1, b2, b3), Vb)  	## multivariate Normal prior on vector of regression params
  prec ~ dgamma(s1,s2)    ## prior precision

  for(i in 1:n){
	  mu[i] <- beta[1] + beta[2]*x1[i] + beta[3]*x2[i] + beta[4]*x1[i]*x2[i]   	## process model - correct?
	  y[i]  ~ dnorm(mu[i], prec)		        ## data model
  }
}
"
...
## specify priors
data <- list()
data$b0 <- b0      ## regression beta means
data$b1 <- b1      ## regression beta means
data$b2 <- b2      ## regression beta means
data$b3 <- b3      ## regression beta means
...
# Run model
j.model.task2  <- jags.model(file = textConnection(multivariate_regression),
                             data = data,
                             inits = inits,
                             n.chains = nchain)

# Sample from the posterior
var.out.task2   <- coda.samples (model = j.model.task2,
                            variable.names = c("beta","prec"),
                                n.iter = 2000)

GBR <- gelman.plot(var.out.task2)

## convert to matrix
var.mat.task2      <- as.matrix(var.out.task2)

## Pairwise scatter plots & correlation
pairs(var.mat.task2)	## pairs plot to evaluate parameter correlation
cor(var.mat.task2)    ## correlation matrix among model parameters

summary(var.out.task2) # No burning or thinning applied
```
